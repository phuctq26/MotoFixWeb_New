                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="form-check mb-2">
                                            <input class="form-check-input" type="radio" name="vehicleOption" 
                                                   ...
                                                   onclick="document.getElementById('vehicleIdInput').value = '<%= v.getVehicleId() %>';
                                                           document.getElementById('newVehicleForm').style.display = 'none';" required>
                    ...
                    <div class="col-lg-4">
                        <div class="card p-4">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <small class="text-muted"><%= new java.text.SimpleDateFormat("dd/MM/yyyy").format(new java.util.Date()) %></small>
                            </div>

                            <c:forEach var="ro" items="${todayOrders}">
                                <div class="schedule-item mb-3">
                                    <div class="fw-semibold text-primary">${ro.plateNumber}</div>
                                    <div class="d-flex justify-content-between mt-2">
                                        <small class="text-muted">${ro.createdAt}</small>

                                        <c:choose>
                                            <c:when test="${ro.status == 'RECEIVED'}">
                                                <span class="badge bg-info text-dark">Chờ sửa</span>
                                            </c:when>
                                            <c:when test="${ro.status == 'IN_PROGRESS'}">
                                                <span class="badge bg-warning text-dark">Đang sửa</span>
                                            </c:when>
                                            <c:when test="${ro.status == 'COMPLETED' || ro.status == 'DONE'}">
                                                <span class="badge bg-success">Xong</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="badge bg-secondary">${ro.status}</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </div>
                                </div>
                            ...
                            <c:if test="${empty todayOrders}">
                                <div class="text-center text-muted py-3">Chưa có xe nào trong hôm nay.</div>
                            </c:if>
                        </div>
                    </div>
                </div>
